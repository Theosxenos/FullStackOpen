...
describe('total likes', () => {
    ...
    test('of empty list is zero', () => {
        const totalSum = totalLikes(emptyBlog);
        expect(totalSum)
            .toBe(0);
    });

    test('when list has only one blog, equals the likes of that', () => {
        const totalSum = totalLikes(listWithOneBlog);
        expect(totalSum)
            .toBe(5);
    });

    test('of a bigger list calculated right', () => {
        const totalSum = totalLikes(listWithMultipleBlogs);
        expect(totalSum)
            .toBe(36);
    });
});
...
describe('blog statistics', () => {
    ...
    test('favorite blog', () => {
        ...
        expect(result)
            .toEqual(listWithMultipleBlogs[2]);
    });
